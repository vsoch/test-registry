...
updated_at: "2023-03-10 03:36:03.430583"
...
```bash
$ shpc install quay.io/biocontainers/encyclopedia
```
...
#### encyclopedia-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### encyclopedia-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/EncyclopeDIA
$ podman run --it --rm --entrypoint /usr/local/bin/EncyclopeDIA   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/EncyclopeDIA   -v ${PWD} -w ${PWD} <container> -c " $@"
```
