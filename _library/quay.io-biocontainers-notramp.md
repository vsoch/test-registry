---
layout: container
name:  "quay.io/biocontainers/notramp"
maintainer: "@vsoch"
...
updated_at: "2023-03-10 03:22:27.996594"
...
```bash
$ shpc install quay.io/biocontainers/notramp
```
...
#### notramp-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### notramp-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/notramp
$ podman run --it --rm --entrypoint /usr/local/bin/notramp   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/notramp   -v ${PWD} -w ${PWD} <container> -c " $@"
```
