---
layout: container
name:  "quay.io/biocontainers/sctagger"
maintainer: "@vsoch"
...
updated_at: "2023-03-10 03:44:04.317692"
...
```bash
$ shpc install quay.io/biocontainers/sctagger
```
...
#### sctagger-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### sctagger-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/scTagger.py
$ podman run --it --rm --entrypoint /usr/local/bin/scTagger.py   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/scTagger.py   -v ${PWD} -w ${PWD} <container> -c " $@"
```
