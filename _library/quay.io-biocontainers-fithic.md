---
layout: container
name:  "quay.io/biocontainers/fithic"
maintainer: "@vsoch"
...
updated_at: "2023-03-09 03:38:27.718282"
...
```bash
$ shpc install quay.io/biocontainers/fithic
```
...
#### fithic-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### fithic-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/fithic
$ podman run --it --rm --entrypoint /usr/local/bin/fithic   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/fithic   -v ${PWD} -w ${PWD} <container> -c " $@"
```
