---
layout: container
name:  "quay.io/biocontainers/olivar"
maintainer: "@vsoch"
...
updated_at: "2023-03-10 03:24:26.297639"
...
```bash
$ shpc install quay.io/biocontainers/olivar
```
...
#### olivar-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### olivar-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/olivar
$ podman run --it --rm --entrypoint /usr/local/bin/olivar   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/olivar   -v ${PWD} -w ${PWD} <container> -c " $@"
```
