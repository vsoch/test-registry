---
layout: container
name:  "quay.io/biocontainers/gunc"
maintainer: "@vsoch"
...
updated_at: "2023-03-09 03:36:47.322837"
...
```bash
$ shpc install quay.io/biocontainers/gunc
```
...
#### gunc-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### gunc-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/gunc
$ podman run --it --rm --entrypoint /usr/local/bin/gunc   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/gunc   -v ${PWD} -w ${PWD} <container> -c " $@"
```
