---
layout: container
name:  "quay.io/biocontainers/woltka"
maintainer: "@vsoch"
...
updated_at: "2023-03-10 03:26:11.788137"
...
```bash
$ shpc install quay.io/biocontainers/woltka
```
...
#### woltka-inspect-runscript:

```bash
$ singularity inspect -r <container>
```

#### woltka-inspect-deffile:
...
```bash
$ singularity exec <container> /usr/local/bin/woltka
$ podman run --it --rm --entrypoint /usr/local/bin/woltka   -v ${PWD} -w ${PWD} <container> -c " $@"
$ docker run --it --rm --entrypoint /usr/local/bin/woltka   -v ${PWD} -w ${PWD} <container> -c " $@"
```
